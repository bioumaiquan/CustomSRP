#ifndef BIOUM_SHADOWS_INCLUDE
#define BIOUM_SHADOWS_INCLUDE
...
{
	float3 normalBias = surfaceWS.originalNormal * directional.normalBias * _CascadeData[global.cascadeIndex].y;
	float3 positionSTS = mul(_DirectionalShadowMatrices[directional.tileIndex], float4(surfaceWS.position + normalBias, 1.0)).xyz;  // convert worldPos to light[index][cascadeIndex] space
	float shadow = FilterDirectionalShadow(positionSTS);
	if(global.cascadeBlend < 1)
	{
		normalBias = surfaceWS.originalNormal * directional.normalBias * _CascadeData[global.cascadeIndex + 1].y;
...
{
    if(directional.strength * global.strength <= 0.001)
    {
